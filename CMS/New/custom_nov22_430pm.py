...
#out = cv2.VideoWriter('output.avi', -1, 1, (desired_w, desired_h))
...
def read_video():
	...
	while True:
		...
		if(t_a_final < 1000):
			cv2.putText(cropped, 'Acceleration TTC: ' + str(round(float(t_a_final),2)) + ' s.', (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0, 0, 255), 3)
			cv2.putText(cropped, 'Momentary TTC: ' + str(round(float(t_m_final),2)) + ' s.', (20, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0, 0, 255), 3)
		...
		if(frameCounter % 15 == 0):
			for tracked_id in carTracker:
				...
				if (width1[tracked_id] != width2[tracked_id] and width1[tracked_id] != 0):
					...
					t_m2[tracked_id] = t
					if(tracked_id in t_m1 and tracked_id in t_m2 and t_m2[tracked_id] < t_m1[tracked_id]):
						...
						if(C < 0):
							...
							print("at {f} when t_a = {t} for ID={_id}".format(f=frameCounter, t=t_a, _id=tracked_id))
							print("at {f} when t_m = {t} for ID={_id}".format(f=frameCounter, t=t, _id=tracked_id))
						else:
							t_a = t

						if(t_m2[tracked_id] < t_m1[tracked_id] and t_a > 0 and t_a_final > t_a):
							#print('t_a = {t}'.format(t=t_a))
							t_a_final = t_a

						if(t_m2[tracked_id] < t_m1[tracked_id] and t > 0 and t_m_final > t):
							#print('t_m = {t}'.format(t=t_m_final))
							t_m_final = t
					#print("TTC for {_id} is {time}".format(_id = tracked_id,
					#										time = t))
						#cv2.putText(cropped, 'TTC: ' + str(int(t_a)) + ' s.', (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0, 0, 255), 3)
				

				# update values
				width1[tracked_id] = width2[tracked_id]
				time1[tracked_id] = time2[tracked_id]
				if(tracked_id in t_m2):
